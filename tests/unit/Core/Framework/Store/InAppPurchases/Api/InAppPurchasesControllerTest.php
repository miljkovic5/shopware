...
class InAppPurchasesControllerTest extends TestCase
{
    ...
    {
        static::expectException(StoreException::class);
        static::expectExceptionMessage('Required In-App Purchase identifier not present.');

        $request = new Request();

        $this->inAppPurchasesController->checkInAppPurchaseActive($request, $this->context);
    }
    ...
    public function testCheckInAppPurchaseActiveWithNonPurchasedAppReturnsFalse(): void
    {
        ...
        $response = $this->inAppPurchasesController->checkInAppPurchaseActive($request, $this->context);
