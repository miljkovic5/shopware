...
class StoreExceptionTest extends TestCase
{
    ...
    {
        $exception = StoreException::missingIdentifier();

        static::assertSame(
            'Required In-App Purchase identifier not present.',
            $exception->getMessage()
        );

        static::assertSame('FRAMEWORK__STORE_MISSING_IDENTIFIER', $exception->getErrorCode());
        static::assertSame(Response::HTTP_BAD_REQUEST, $exception->getStatusCode());
    }
